...
class serialThread (QThread):
    ...
    def run(self):
        reData = re.compile(b"DATA")
        reTime = re.compile(b"TIME")
        reEnd = re.compile(b"END")
        self.getJson()
        ...
        if(self.serialConnection.is_open):
            ...
            while self._isRunning:
                ...
                entry = 1
                #print("In buffer", self.serialConnection.in_waiting)
                #entry = self.getEntry()
                bytestoread = self.serialConnection.in_waiting
                if(bytestoread):
                    inputBuf = self.serialConnection.read(bytestoread)
                    self.serialInBuffer = self.serialInBuffer + inputBuf
                endpos = reEnd.search(self.serialInBuffer)
                index = 0
                if endpos != None:
                    for endpos in reEnd.finditer(self.serialInBuffer):
                        entrypos = reData.search(self.serialInBuffer, index)
                        entrytimepos = reTime.search(self.serialInBuffer, index)
                        slave_pos = self.serialInBuffer[entrypos.end()]
                        sensor_pos = self.serialInBuffer[entrypos.end()+1]
                        sizeofdata = (self.serialInBuffer[entrypos.end()+2]+self.serialInBuffer[entrypos.end()+3]*256)*2
                        sizeoftime = (self.serialInBuffer[entrytimepos.end()]+self.serialInBuffer[entrytimepos.end()+1]*256)*2
                        data = self.serialInBuffer[entrypos.end()+3:entrypos.end()+3+sizeofdata]
                        tempo = self.serialInBuffer[entrytimepos.end()+1:entrytimepos.end()+1+sizeoftime]
                        index = endpos.end()
                        if not entrypos.end()+4+sizeofdata == entrytimepos.start():
                            entry = 1
                            print("Data size doesnt match with time start")
                            continue
                        if not entrytimepos.end() + 2 + sizeoftime == endpos.start():
                            entry = 1
                            print("Time size doesnt match with end start")
                            continue
                        if not sizeofdata == sizeoftime:
                            entry = 1
                            print("Data size and TIme size not equal")
                            continue
                        ...
                        # convert data from byte array to int list
                        # print("=================STARTED==================")
                        for c in range(0, sizeofdata, 2):
                            sData.append(data[c] + data[c + 1] * 256)
                        for c in range(0, sizeoftime, 2):
                            segundos = tempo[c + 1] >> 2
                            milisegundos = (tempo[c + 1] & 3) * 256 + tempo[c]
                            sTime.append(segundos * 1000 + milisegundos)
                        # print("=================FINISHED==================")
                        entry = (slave_pos, sensor_pos, sData, sTime)
                    ...
                    self.serialInBuffer = self.serialInBuffer[endpos.end():]
                    print("Bytes to process", len(self.serialInBuffer))
